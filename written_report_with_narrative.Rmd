...
Notes for editing:
Two of the predictors have few unique observations and St already seems linear. Therefore, we used only a 2nd degree polynomial due to the fact that a 2nd degree polynomial needs at least 3 unique values to solve. Because the polynomial is relatively low degree, we are not worried about erratic behavior and did not use a spline or smoothing method (when we used these, they did not offer much better performance in terms of R^2 and error reduction). To increase our flexibility and capture complex behavior between interactions, we considered many interactions, up to degree two.
